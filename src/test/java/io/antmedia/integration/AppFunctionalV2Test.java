	...
	public void testSendRTMPStream() {
		...
		try {
			...
			if(isEnterprise) {
				...
				assertTrue(MuxingTest.testFile("http://" + SERVER_ADDR + ":5080/LiveApp/streams/" + broadcast.getStreamId() + "_240p_300kbps.m3u8"));
				assertTrue(MuxingTest.testFile("http://" + SERVER_ADDR + ":5080/LiveApp/streams/" + broadcastWithSubFolder.getSubFolder()+ "/" +  broadcastWithSubFolder.getStreamId() + "_240p_300kbps.m3u8"));

				Awaitility.await().atMost(10, TimeUnit.SECONDS).pollInterval(1, TimeUnit.SECONDS).until(() -> {
					return MuxingTest.testFile("http://" + SERVER_ADDR + ":5080/LiveApp/streams/" + broadcast.getStreamId() + "_240p_300kbps.mp4");
				});
				Awaitility.await().atMost(10, TimeUnit.SECONDS).pollInterval(1, TimeUnit.SECONDS).until(() -> {
					return MuxingTest.testFile("http://" + SERVER_ADDR + ":5080/LiveApp/streams/" + broadcastWithSubFolder.getSubFolder()+ "/" +   broadcastWithSubFolder.getStreamId() + "_240p_300kbps.mp4");
				});
			...
			for (VoD voD : callGetVoDList) {
				if (voD.getStreamId().equals(broadcast.getStreamId())) 
				{
					...
					else if (voD.getFilePath().equals("streams/"+broadcast.getStreamId() + "_240p_300kbps.mp4")) {
						vod2 = voD;
					}
					...
					else if (voD.getFilePath().equals("streams/" + broadcastWithSubFolder.getSubFolder() + "/" + broadcastWithSubFolder.getStreamId() + "_240p_300kbps.mp4")) {
						vod4 = voD;
					}
