...
public class ConsoleAppRestServiceTest {
	...
	public void testACreateInitialUser() {
		try {
			...
			assertTrue("Server is not started from scratch. Please delete server.db file and restart server", firstLogin.isSuccess());
	...
	public void testAllowOnlyStreamsInDataStore() {
		try {
			...
			AppFunctionalTest.executeProcess(ffmpegPath + " -re -i src/test/resources/test.flv -acodec copy -vcodec copy -f flv rtmp://localhost/LiveApp/" + streamId);
	...
	private Result callSetAppSettings(String appName, AppSettingsModel appSettingsModel) throws Exception {
		...
		HttpUriRequest post = RequestBuilder.post()
				.setUri(url)
				.setHeader(HttpHeaders.CONTENT_TYPE, "application/json")
				.setEntity(new StringEntity(gson.toJson(appSettingsModel)))
				.build();
