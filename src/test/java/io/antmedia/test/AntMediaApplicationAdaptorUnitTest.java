...
public class AntMediaApplicationAdaptorUnitTest {
	...
	@Test
	public void testSendUnexpectedShutdownHook(){
		IScope scope = mock(IScope.class);
		when(scope.getName()).thenReturn("junit");
		...
		spyAdapter.setScope(scope);
		AppSettings appSettings = new AppSettings();
		appSettings.setListenerHookURL("test-url");
		spyAdapter.setAppSettings(appSettings);
		spyAdapter.setStreamPublishSecurityList(new ArrayList<>());
		...
		Awaitility.await().atMost(5, TimeUnit.SECONDS).until(()-> {
			...
			try {
				//verify that notifyHook is called 1 time
				verify(spyAdapter, times(1)).notifyHook(appSettings.getListenerHookURL(), null, AntMediaApplicationAdapter.HOOK_UNEXPECTED_SERVER_SHUTDOWN, null, null,
						null, null, null);
				called = true;
			} catch (Exception e) {
				e.printStackTrace();
	...
	@Test
	public void testSendHighResourceUsageHook() throws IOException {
		IScope scope = mock(IScope.class);
		when(scope.getName()).thenReturn("junit");
		...
		when(context.getBean(StatsCollector.BEAN_NAME)).thenReturn(statsCollector);

		when(statsCollector.enoughResource()).thenReturn(false);

		final Queue<IScope> scopes = new LinkedList<>();
		scopes.add(scope);
		Mockito.doReturn("resourceUsageData").when(spyAdapter).getSystemResourcesInfo(scopes);
		...
		Awaitility.await().pollInterval(1,TimeUnit.SECONDS).atMost(
				2, TimeUnit.SECONDS).until(()-> {
					System.out.println("checking...");
			boolean called = false;
			try {
				verify(spyAdapter, times(1)).notifyHook(appSettings.getListenerHookURL(), null, AntMediaApplicationAdapter.HOOK_HIGH_RESOURCE_USAGE, null, null,
						null, null, "resourceUsageData");
				called = true;
			} catch (Exception e) {
				e.printStackTrace();
			}
