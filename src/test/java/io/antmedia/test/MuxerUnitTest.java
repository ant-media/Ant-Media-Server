...
public class MuxerUnitTest extends AbstractJUnit4SpringContextTests {
	...
	public void testVideoOnly() {
		testSingleStreamOnly("target/test-classes/test_video.flv", false, true);
	}
	
	@Test
	public void testAudioOnly() 
	{
		testSingleStreamOnly("target/test-classes/test_audio.flv", true, false);
	}
	
	
	public void testSingleStreamOnly(String fileName, boolean expectedAudioExistence, boolean expectedVideoExistence) 
	{
		if (appScope == null) {
			appScope = (WebScope) applicationContext.getBean("web.scope");
			logger.debug("Application / web scope: {}", appScope);
		...
		String name = "single_stream_single_" + (int)(Math.random()*100000);
		...
		new Thread() {
			...
			public void run() {
				try {
					...
					while (flvReader.hasMoreTags()) {
						ITag readTag = flvReader.readTag();
						long timeDiff = readTag.getTimestamp() - (System.currentTimeMillis() - now);
		...
		assertEquals(expectedAudioExistence, muxAdaptor.isEnableAudio());
		assertEquals(expectedVideoExistence, muxAdaptor.isEnableVideo());

		logger.info("Stream info find time: {} audio exists:{} video exists:{}", muxAdaptor.getStreamInfoFindTime(), muxAdaptor.isEnableAudio(), muxAdaptor.isEnableVideo());

			
		assertTrue(muxAdaptor.getStreamInfoFindTime() < 10000);
			
		Awaitility.await().atMost(30, TimeUnit.SECONDS).until(()-> {
			return scheduler.getScheduledJobNames().isEmpty();
		});
