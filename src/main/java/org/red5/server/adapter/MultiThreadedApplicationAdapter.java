 ...
 * connections and scheduling services. <br>
 ...
 * side with the following code: <br>
 * 
 * <code>
...
public class MultiThreadedApplicationAdapter extends StatefulScopeWrappingAdapter implements ISharedObjectService,
		...
		if (plugins != null) {
			for (PluginDescriptor desc : plugins) {
				...
				try {
					...
					// if the plugin class extends the red5 plug-in we will add
					// the application to it
					if (plugin instanceof Red5Plugin) {
						// pass the app to the plugin so that it may be
						// manipulated directly by the plug-in
					...
					if (desc.getMethod() != null) {
						...
						// if a return type is not specified for the plug-in
						// method just invoke it
	...
	public boolean hasBroadcastStream(IScope scope, String name) {
		IProviderService service = (IProviderService) ScopeUtils.getScopeService(scope, IProviderService.class,
	...
	/**
	 * Returns list of stream names broadcasted in scope. Broadcast stream name
	 * is somewhat different from server stream name. Server stream name is just
	 * an ID assigned by Red5 to every created stream. Broadcast stream name is
	 * the name that is being used to subscribe to the stream at client side,
	 * that is, in
	 * 
	 * <code>
	 * NetStream.play
	...
	public IOnDemandStream getOnDemandStream(IScope scope, String name) {
		...
		IOnDemandStreamService service = (IOnDemandStreamService) ScopeUtils.getScopeService(scope,
				IOnDemandStreamService.class, StreamService.class, false);
	...
	public ISubscriberStream getSubscriberStream(IScope scope, String name) {
		log.warn("This won't work until the refactoring of the streaming code is complete.");
		ISubscriberStreamService service = (ISubscriberStreamService) ScopeUtils.getScopeService(scope,
	...
	public double getStreamLength(String name) {
		double duration = 0;
		IProviderService provider = (IProviderService) ScopeUtils.getScopeService(scope, IProviderService.class,
				ProviderService.class);
	...
	/**
	 * Start transmission notification from Flash Player 11.1+. This command
	 * asks the server to transmit more data because the buffer is running low.
	 * 
	...
	/**
	 * Stop transmission notification from Flash Player 11.1+. This command asks
	 * the server to suspend transmission until the client sends a startTransmit
	 * event because there is enough data in the buffer.
	 */
	...
	/**
	 * Notification method that is sent by some clients just before playback
	 * starts.
	 * 
	 * @param streamName
	...
	public void streamBroadcastClose(IBroadcastStream stream) {
		...
		if (conn != null) {
			...
			log.info("W3C x-category:stream x-event:unpublish x-sname:{} x-file-length:{} x-name:{}",
		...
		if (recordingName != null) {
			if (conn != null) {
				...
				log.info("W3C x-category:stream x-event:recordstop x-sname:{} x-file-name:{} x-file-length:{}",
						new Object[] { stream.getName(), recordingName, publishDuration });
			}
			// if the stream length is 0 bytes then delete it, this is a fix for
			// SN-20
			// get the web root
	...
	public void streamPlayItemStop(ISubscriberStream stream, IPlayItem item) {
		...
		if (log.isInfoEnabled()) {
			...
			if (stream instanceof PlaylistSubscriberStream) {
				// converted to seconds
				playDuration = (System.currentTimeMillis() - ((PlaylistSubscriberStream) stream).getCreationTime())
						/ 1000;
			}
			long playItemSize = -1;
			String playItemName = "";
			...
			log.info(
					...
					new Object[] { remoteAddress, readBytes, writtenBytes, stream.getName(), playDuration, playItemSize,
							playItemName });
		}
	}
	...
	public void streamPlayItemPause(ISubscriberStream stream, IPlayItem item, int position) {
		// log w3c connect event
		log.info("W3C x-category:stream x-event:pause c-ip:{} x-sname:{}", Red5.getConnectionLocal().getRemoteAddress(),
				stream.getName());
	}
