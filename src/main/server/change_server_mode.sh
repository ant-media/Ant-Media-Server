...
usage() {
  ...
  echo ""
  echo "You can use connection string directly as {MONGO_DB_SERVER} that starts with mongodb:// or mongodb+srv://"
  ...
  echo ""
  echo "Pay attention: If you use connection string directly - contains username and password - as {MONGO_DB_SERVER} parameter like above, then Ant Media Server ignores {MONGO_DB_USERNAME} and {MONGO_DB_PASSWORD}."
  echo ""
  echo ""
...
sed -i $SED_COMPATIBILITY 's/useGlobalIp=.*/useGlobalIp='$USE_GLOBAL_IP'/' $AMS_INSTALL_LOCATION/conf/ant-media-web.properties
